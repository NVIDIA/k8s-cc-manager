...
_reset_gpu_after_cc_mode() {
    local gpu=$1
    python3 /usr/bin/gpu-admin-tools/nvidia_gpu_tools.py --reset-after-cc-mode-switch --gpu-bdf=$gpu
    if [ $? -ne 0 ]; then
...
_assert_gpu_cc_mode() {
    ...
    output=$(python3 /usr/bin/gpu-admin-tools/nvidia_gpu_tools.py --query-cc-mode --gpu-bdf=$gpu 2>&1)
    if [ $? -ne 0 ]; then
        _exit_failed
    fi
...
set_gpu_cc_mode() {
    ...
    if ! _assert_gpu_cc_mode $gpu $mode; then
        output=$(python3 /usr/bin/gpu-admin-tools/nvidia_gpu_tools.py --set-cc-mode=$mode --reset-after-cc-mode-switch --gpu-bdf=$gpu 2>&1)
        if [ $? -ne 0 ]; then
